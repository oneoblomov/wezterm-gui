...
# Color mappings for color schemes - these should match actual WezTerm color schemes
color_mappings = {
...
# Get available color schemes from the color mappings
color_scheme_options = list(color_mappings.keys())

# State initialization for theme and custom colors
def initialize_session_state():
    ...
    # Diğer gerekli durumlar da eklenebilir
...
def handle_error(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            error_msg = f"{func.__name__} çalıştırılırken hata oluştu: {str(e)}"
...
def get_colors_for_theme(theme, color_scheme, custom_colors=None):
    """Get correct colors based on theme and color scheme"""
    if theme == "Custom" and custom_colors:
        return custom_colors
    return color_mappings.get(color_scheme, color_mappings['Builtin Dark'])

# Helper function to get WezTerm config path based on OS - Streamlit Cloud uyumlu
def get_wezterm_config_path():
    """Return the expected WezTerm configuration file path based on the current OS"""
    try:
        ...
        st.error(f"Yapılandırma yolu belirlenirken hata: {e}")
        return None

# Helper function to save configuration - Streamlit Cloud uyumlu
@handle_error
def save_configuration_to_wezterm(lua_code):
    """Save the configuration directly to the WezTerm config location"""
...
# Helper function to generate HTML components for terminal preview
def generate_terminal_html_components(colors, enable_tab_bar, use_fancy_tab_bar, 
    ...
    """Generate HTML components for terminal preview (tab bar, cursor, scrollbar, etc.)"""
    components = {}
...
# Generate settings table for preview
...
# Lua configuration code generator
@handle_error
...
# Terminal preview HTML generator
@handle_error
def generate_terminal_preview(theme, font, font_size, color_scheme, custom_colors=None, opacity=0.95,
    ...
    try:
        ...
        components = generate_terminal_html_components(
...
<span style="color: {colors['prompt']};">user@machine</span><span style="color: {colors['fg']};">:</span><span style="color: #5f87ff;">~/projects</span><span style="color: {colors['prompt']};">$</span> <span style="{components['cursor']}">_</span>
        ...
        # Add settings table
        terminal_html += generate_settings_table(
...
# Function to import color scheme from JSON
@handle_error
def import_color_scheme(file_content):
    """Import color scheme from JSON file content and update session state"""
...
# Import/export functionality for color schemes
...
with import_export:
    ...
    if st.button("Mevcut Renk Şemasını Dışa Aktar"):
        try:
            ...
            if st.session_state['theme'] == 'Custom':
                ...
                export_data = color_mappings.get(color_scheme, {})
            export_json = json.dumps(export_data)
            st.code(export_json)
            st.download_button(
                "JSON İndir", 
                export_json, 
                file_name=f"{color_scheme if st.session_state['theme'] != 'Custom' else 'custom'}_colors.json",
    ...
    if upload_file is not None:
        try:
            success = import_color_scheme(upload_file.getvalue().decode())
            if success:
...
# Create two columns: one for the configuration code and one for the preview
...
with col1:
    ...
    if theme == 'Custom':
        lua_code = generate_wezterm_lua(theme, font, font_size, color_scheme, custom_colors, opacity,
                                     enable_tab_bar, enable_scroll_bar, cursor_style,
                                     padding, line_height, use_fancy_tab_bar, hyperlinkRules,
                                     leader_key)
    else:
        lua_code = generate_wezterm_lua(theme, font, font_size, color_scheme, None, opacity,
                                     enable_tab_bar, enable_scroll_bar, cursor_style,
                                     padding, line_height, use_fancy_tab_bar, hyperlinkRules,
                                     leader_key)
    
    if lua_code:
        # Display the Lua code in a code block
        ...
        if not is_cloud:
            if st.button("Doğrudan WezTerm Yapılandırma Konumuna Kaydet"):
                success, result = save_configuration_to_wezterm(lua_code)
                if success:
                    st.success(f"Yapılandırma başarıyla kaydedildi: {result}")
                else:
                    st.error(f"Yapılandırma kaydedilemedi: {result}")
        
        # Kullanım talimatları - Streamlit Cloud'a göre uyarlanmış
        st.info("""
...
with col2:
    ...
    try:
        # Generate terminal preview based on settings
        if st.session_state['theme'] == 'Custom':
            terminal_preview = generate_terminal_preview(
                st.session_state['theme'], font, font_size, color_scheme, 
                st.session_state['custom_colors'], opacity,
                enable_tab_bar, enable_scroll_bar, cursor_style,
                padding, line_height, use_fancy_tab_bar, hyperlinkRules,
                leader_key
            )
            ...
            terminal_preview = generate_terminal_preview(
                st.session_state['theme'], font, font_size, color_scheme, None, opacity,
                enable_tab_bar, enable_scroll_bar, cursor_style,
                padding, line_height, use_fancy_tab_bar, hyperlinkRules,
                leader_key
            )
        ...
        if terminal_preview:
            components.html(terminal_preview, height=650)  # Increased height for detailed preview
        else:
